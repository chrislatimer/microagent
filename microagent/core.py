...
class Microagent:
    ...
    def run(
        ...
        while len(history) - init_len < max_turns and active_agent:
            ...
            # Handle different API responses and convert to a mutable dictionary
            if self.llm_type == 'openai':
                message = completion.choices[0].message.model_dump()
            elif self.llm_type == 'anthropic':
                message = {"role": "assistant", "content": completion.content[0]}
            ...
            # Handle function calls, updating context_variables, and switching agents
            partial_response = self.handle_tool_calls(
                message.get('tool_calls', []), active_agent.functions, context_variables, debug
            )
            history.extend(partial_response.messages)
            context_variables.update(partial_response.context_variables)
            if partial_response.agent:
